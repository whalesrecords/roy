        ...
        <div className="divide-y divide-divider max-h-[500px] overflow-y-auto">
          ...
          {contracts
            ...
            .map((contract) => {
              const { artistShare } = getContractShares(contract, artistId);
              return (
                <div key={contract.id} className="px-4 py-3 flex items-center justify-between gap-3 bg-warning-50/40">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-warning/10 text-warning-700">Release</span>
                      <span className="text-sm font-medium text-secondary-500 font-mono">{contract.scope_id}</span>
                    </div>
                    <span className="text-xs text-warning-600">Pas de ventes importées</span>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-success/10 text-success-700">{formatPercent(artistShare)}%</span>
                    ...
                    <button onClick={() => contract.id && handleDeleteContract(contract.id)} disabled={deletingContractId === contract.id} className="p-1.5 text-secondary-400 hover:text-danger hover:bg-danger-50 rounded transition-colors" title="Supprimer">
