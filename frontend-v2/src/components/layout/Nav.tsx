...
export default function Nav() {
    ...
    <>
      ...
      <div className="px-4 py-5 flex items-center gap-3">
        {labelSettings?.logo_base64 ? (
          <img src={labelSettings.logo_base64} alt="Logo" className="h-8 w-8 object-contain" />
        ) : (
          <div className="w-8 h-8 bg-gradient-to-br from-primary-600 to-primary rounded-xl flex items-center justify-center shadow-lg shadow-primary/30">
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 group ${
                active
                  ? 'bg-gradient-to-r from-primary-600 to-primary text-white shadow-md shadow-primary/25'
                  : 'text-secondary-500 hover:bg-content2 hover:text-foreground'
              }`}
            >
              <svg className={`w-5 h-5 flex-shrink-0 ${active ? 'text-white' : 'text-secondary-400 group-hover:text-primary'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="px-3 py-3 border-t border-divider space-y-1">
        ...
        <button
          onClick={toggleTheme}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-secondary-500 hover:bg-content2 hover:text-foreground transition-all"
        >
          {theme === 'light' ? (
  ...
  return (
    <>
      {/* Mobile top bar */}
      <header className="lg:hidden fixed top-0 left-0 right-0 z-50 h-14 bg-content1/95 backdrop-blur-md border-b border-divider flex items-center justify-between px-4">
        <button
          onClick={() => setIsMobileOpen(!isMobileOpen)}
          className="p-2 rounded-xl bg-content2 text-secondary-500 hover:bg-content3 transition-colors"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            {isMobileOpen ? (
        ...
        <div className="flex items-center gap-2">
          {labelSettings?.logo_base64 ? (
            <img src={labelSettings.logo_base64} alt="Logo" className="h-7 w-7 object-contain" />
          ) : (
            <div className="w-7 h-7 bg-gradient-to-br from-primary-600 to-primary rounded-lg flex items-center justify-center">
              <span className="text-white font-bold text-xs">W</span>
            </div>
          )}
          <span className="font-bold text-foreground">Royalties</span>
        </div>
        ...
        <div className="relative" ref={notifRef}>
          <button
            ...
            className={`p-2 rounded-xl transition-colors relative ${
              isNotifOpen ? 'bg-primary text-white' : 'bg-content2 hover:bg-content3 text-secondary-500'
            }`}
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {isNotifOpen && (
            <div className="absolute right-0 top-full mt-2 w-80 bg-content1 rounded-2xl shadow-xl border border-divider overflow-hidden z-50">
              ...
              <div className="max-h-96 overflow-y-auto">
                {notifications.length === 0 ? (
                  <div className="p-6 text-center text-secondary-500 text-sm">Aucune notification</div>
                ) : (
                  notifications.slice(0, 10).map((notif) => (
                    <div
                      key={notif.id}
                      className={`p-3 border-b border-divider last:border-0 hover:bg-content2 cursor-pointer transition-colors ${
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-foreground truncate">{notif.artist_name || 'Système'}</p>
                          <p className="text-xs text-secondary-500 mt-0.5">{notif.message}</p>
                          <p className="text-xs text-secondary-400 mt-1">{formatTimeAgo(notif.created_at)}</p>
                        </div>
                        {!notif.is_read && (
      ...
      <aside className={`fixed top-0 left-0 z-50 h-full bg-content1 border-r border-divider flex flex-col transition-all duration-300 ${
      ...
      {/* Desktop notification button (top-right floating) */}
      <div className="hidden lg:block fixed top-4 right-4 z-40" ref={notifRef}>
        <button
          onClick={() => setIsNotifOpen(!isNotifOpen)}
          className={`p-2.5 rounded-xl transition-all shadow-sm ${
            isNotifOpen
              ? 'bg-primary text-white shadow-primary/25'
              : 'bg-content1 hover:bg-content2 text-secondary-500 border border-divider'
          }`}
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
        ...
        {isNotifOpen && (
          <div className="absolute right-0 top-full mt-2 w-80 bg-content1 rounded-2xl shadow-xl border border-divider overflow-hidden">
            <div className="p-3 border-b border-divider flex items-center justify-between">
              <h3 className="font-semibold text-foreground">Notifications</h3>
              {unreadCount > 0 && (
                <button onClick={handleMarkAllRead} className="text-xs text-primary hover:underline">
