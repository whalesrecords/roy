 ...
 * (settings/label, variante sombre incluse) ; sinon repli sur le logo EMBARQUÉ
 * thème-aware — donc il s'affiche toujours.
 */
export function LabelLogo({ height = 26, maxWidth = 160 }: { height?: number; maxWidth?: number }) {
  ...
  const logoLight = ls?.logo_base64 || ls?.logo_url;
  ...
  useEffect(() => {
    if (!src) return;
    let cancelled = false;
    Image.getSize(src, (w, h) => { if (!cancelled && h > 0) setRatio(w / h); }, () => {});
    return () => { cancelled = true; };
  }, [src]);
