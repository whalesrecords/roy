    ...
    <div className="min-h-screen bg-background safe-top safe-bottom">
      ...
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-divider">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <div>
            <h1 className="font-semibold text-foreground">Mes Tracks</h1>
            <p className="text-xs text-secondary-500">{tracks.length} titre{tracks.length > 1 ? 's' : ''}</p>
          </div>
        </div>
      </header>
      ...
      <main className="px-4 py-4 pb-24 space-y-2">
        ...
        {tracks.length === 0 && !error && (
          <div className="text-center py-12">
            ...
            <p className="text-secondary-500">Aucun track pour le moment</p>
          </div>
        )}
